...
def _mirror_obs(obs: torch.Tensor) -> torch.Tensor:
    ...
    gait_phase = obs[..., 53:54] # gait phase
    gait_clock = obs[..., 54:58] # gait clock for each leg
    ...
    gait_clock_m = gait_clock.index_select(dim=-1, index=torch.tensor([1,0,3,2], device=obs.device))

    return torch.cat([w_m, g_m, q_leg_m, dq_m, actions_m, cmd_m, gait_phase, gait_clock_m], dim=-1)
...
def _mirror_privileged_obs(privileged_obs: torch.Tensor) -> torch.Tensor:
    ...
    gait_phase = privileged_obs[..., 53:54] # gait phase
    gait_clock = privileged_obs[..., 54:58] # gait clock for each leg
    ...
    dq_m = _mirror_dof_all(dq)
    actions_m = _mirror_dof_all(actions)
    gait_clock_m = gait_clock.index_select(dim=-1, index=torch.tensor([1,0,3,2], device=privileged_obs.device))

    base_vel = privileged_obs[..., 58:61] # base_lin_vel
    base_vel_m = base_vel * base_vel.new_tensor([1.0, -1.0, 1.0])

    foot_contact_forces = privileged_obs[..., 61:73] # foot_contact_forces 4*3
    ...
    tau = privileged_obs[..., 73:89] # joint_torques
    tau_m = _mirror_dof_all_orig(tau)

    nx, ny = _get_height_grid_shape()
    heights = privileged_obs[..., 89:]
