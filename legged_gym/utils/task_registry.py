...
class TaskRegistry():
    ...
    def make_alg_runner(self, env, name=None, args=None, train_cfg=None, log_root="default") -> Tuple[OnPolicyRunner, LeggedRobotCfgPPO]:
        ...
        if resume:
            ...
            if train_cfg.runner.resume_path is not None:
                ...
                resume_path = resume_path.replace("{LEGGED_GYM_ROOT_DIR}", LEGGED_GYM_ROOT_DIR)
            else:
    ...
    def save_cfgs(self, name):
        ...
        save_items = [
            os.path.join(self.log_dir, LEGGED_GYM_ENVS_DIR + "/base"),
            os.path.join(self.log_dir, LEGGED_GYM_ENVS_DIR + "/" + name),
        ]

        if save_items is not None:
            for save_item in save_items:
                base_name = os.path.basename(save_item)
                dst_path = os.path.join(self.log_dir, base_name)

                if os.path.isfile(save_item):
                    shutil.copy2(save_item, dst_path)
                elif os.path.isdir(save_item):
                    shutil.copytree(save_item, dst_path, dirs_exist_ok=True)
                else:
                    print(f"路径不存在: {save_item}")
